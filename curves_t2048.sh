...
' "$LOG")
python plot_lm.py "$LOG" --ref "$REF" --out plot/long16h_t2048.png 2>&1 | grep -vE "UserWarning|ax2\.axhline"
echo
python plot_lm.py "$LOG" --ref "$REF" --out plot/long16h_t2048_wallclock.png --wallclock 2>&1 | grep -vE "UserWarning|ax2\.axhline"
echo
python -c '
import json, sys
rows = {}

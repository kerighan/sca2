...
class LaplaceConfig:
    ...
    theta_scale: float = 0.02      # init scale of the content phase (0 -> content path is dead)
    persist: float = 0.5           # fraction of long-head modes pinned at lambda = 0 (lowest freqs)
    mem_range: Tuple[float, float] = (64.0, 4096.0)   # init memories 1/lambda of the damped modes
    ...
    long_path: str = "batched"     # "batched" (intra-chunk work for all chunks at once) | "chunk"
    gemm_dtype: Optional[torch.dtype] = None   # dtype of the two big long-head GEMMs; None = autocast/fp32
...
class _NoAutocast:
    ...
    def __enter__(self):
        self.ctx = torch.autocast(device_type=self.dev, enabled=False); self.ctx.__enter__()
    def __exit__(self, *a):
        self.ctx.__exit__(*a)
...
class LongHead(nn.Module):
    def __init__(self, cfg: LaplaceConfig):
        ...
        self.theta = nn.Parameter(torch.zeros(M) if cfg.theta_scale == 0.0
        ...
        nn.init.zeros_(self.bproj.weight); nn.init.constant_(self.bproj.bias, cfg.beta_init)
        lo, hi = cfg.mem_range
        mem = torch.exp(torch.empty(M).uniform_(math.log(lo), math.log(hi)))
        self.lam_raw = nn.Parameter(torch.log(torch.expm1(1.0 / mem)))    # softplus^{-1}(lambda)
        mask = torch.ones(M)
    ...
    def lam(self) -> torch.Tensor:
        return F.softplus(self.lam_raw.to(self.wd)).clamp(max=self.cfg.lam_max) * self.lam_mask
    ...
    def init_state(self, B: int, device) -> State:
        ...
        return {"sr": z(), "si": z(), "pos": torch.zeros((), device=device, dtype=self.wd)}
    ...
    def _chunk(self, z, h, st: State):
        ...
        with _NoAutocast(dev):
            ...
            dT, gT, d1 = torch.exp(-lam * T)[:, None], torch.exp(-lam * (T - 1))[:, None], torch.exp(-lam)[:, None]
            p = idx + st["pos"]
            pw, pq = self._phase(h, p), self._phase(z, p)                  # (B,T,M) fp32
            cw, sw, cq, sq = pw.cos(), pw.sin(), pq.cos(), pq.sin()
            cw_w, sw_w, cw_q, sw_q, cq_q, sq_q = cw * gw, sw * gw, cw * gq, sw * gq, cq * gq, sq * gq
            ...
            r = (torch.einsum("btm,bmj->btj", cw_q, sr0) + torch.einsum("btm,bmj->btj", sw_q, si0)) / M
            e = torch.linalg.solve_triangular(A, self.V(z).to(self.wd) - beta * r, upper=False, unitriangular=True)
            Am, Bm = self.wr * cw_w - self.wi * sw_w, self.wr * sw_w + self.wi * cw_w
            Fq = torch.cat([cq_q, sq_q], -1)                                 # (B,T,2M)
            Fk = torch.cat([torch.cat([Am, Bm], -1), torch.cat([Bm, -Am], -1)], 1)   # (B,2T,2M)
        ...
        with _NoAutocast(dev):
            ...
            upd = torch.cat([cw_w, sw_w], -1).transpose(1, 2) @ e * torch.cat([gT, gT], 0)
    ...
    def _batched(self, z, h, st: State, K: int):
        ...
        with _NoAutocast(dev):
            ...
            dC, gT, d1 = torch.exp(-lam * C)[:, None], torch.exp(-lam * (C - 1))[:, None], torch.exp(-lam)[:, None]
            ...
            W = torch.linalg.solve_triangular(eye + beta * G.tril(-1), eye.expand(B, K, C, C),
                                              upper=False, unitriangular=True)         # (B,K,C,C)
            Am, Bm = self.wr * cw_w - self.wi * sw_w, self.wr * sw_w + self.wi * cw_w
            Fq = torch.cat([cq_q, sq_q], -1)
            Fk = torch.cat([torch.cat([Am, Bm], -1), torch.cat([Bm, -Am], -1)], 2)
            Rq = torch.cat([cw_q, sw_q], -1)
            Cq = torch.cat([self.wr * cq_q + self.wi * sq_q, self.wr * sq_q - self.wi * cq_q], -1)
            Pw = torch.cat([cw_w, sw_w], -1).transpose(-1, -2)
            gT2 = torch.cat([gT, gT], 0)
        gd = self._gemm_dtype(z)
        K2 = (Fq.to(gd) @ Fk.to(gd).transpose(-1, -2)).view(B, K, C, 2, C)
        K2 = K2.masked_fill(_causal_mask(C, dev)[None, None, :, None, :], 0).reshape(B, K, 2 * C, C)
        ...
        for k in range(K):
            ...
            with _NoAutocast(dev):
                S4 = torch.cat([torch.cat([sr0, si0], -1), torch.cat([si0, -sr0], -1)], 1)
                o = o + (Cq[k] @ S4) / M
                upd = (Pw[k] @ e) * gT2
    ...
    def step(self, z_t, h_t, state: State):
        with _NoAutocast(z_t.device):
            ...
            vhat = (torch.einsum("bm,bmj->bj", cwt, sr0) + torch.einsum("bm,bmj->bj", swt, si0)) / M
            ...
            m = torch.einsum("bam,bcmj->bacj", torch.stack([c1, c2], 1), torch.stack([sr, si], 1)) / M
...
class ShortHead(nn.Module):
    def __init__(self, cfg: LaplaceConfig):
        ...
        self.wr = nn.Parameter(torch.ones(L))       # w = 1: delta at lag 0 at init (o_t = V(z_t))
        self.wi = nn.Parameter(torch.zeros(L))
        # built in float64: 2*pi/L rounded in float32 breaks the comb's exact cancellation
        self.register_buffer("omega", (torch.arange(L, dtype=torch.float64) * (2 * math.pi / L)).float())
    ...
    def init_state(self, B: int, device) -> State:
        ...
        return {"c": torch.ones(B, n, L, device=device, dtype=wd), "s": torch.zeros(B, n, L, device=device, dtype=wd),
                "e": torch.zeros(B, n, self.dv, device=device, dtype=wd),
                "pos": torch.zeros((), device=device, dtype=torch.long)}

    def _phase(self, x, p):
        return self.K(x).to(self.wd) * self.theta.to(self.wd) + (p % self.L).to(self.wd)[..., None] * self.omega
    ...
    def _read(self, cq, sq, cw, sw, e):
        ...
        return torch.cat([torch.einsum("btw,btwj->btj", k_re, e), torch.einsum("btw,btwj->btj", k_im, e)], -1)
    ...
    def prefill(self, z, h, state: Optional[State] = None):
        ...
        with _NoAutocast(z.device):
            ...
            cw = torch.cat([st["c"], phi.cos()], 1); sw = torch.cat([st["s"], phi.sin()], 1)
    ...
    def step(self, z_t, h_t, state: State):
        with _NoAutocast(z_t.device):
            ...
            cw = torch.cat([state["c"], phi.cos()[:, None]], 1); sw = torch.cat([state["s"], phi.sin()[:, None]], 1)
            e = torch.cat([state["e"], self.V(z_t).to(self.wd)[:, None]], 1)
            psi = self._phase(z_t, p)
            u = self._read(psi.cos()[:, None], psi.sin()[:, None], cw[:, None], sw[:, None], e[:, None])[:, 0]
        return _rms(u).to(z_t.dtype), {"c": cw[:, 1:], "s": sw[:, 1:], "e": e[:, 1:], "pos": state["pos"] + 1}
...
class LaplaceAttention(nn.Module):
    ...
    def init_state(self, B: int, device) -> State:
        return {"long": self.long.init_state(B, device), "short": self.short.init_state(B, device),
                "z_prev": torch.zeros(B, self.cfg.d, device=device, dtype=self.n.weight.dtype)}
...
def _verify_against_repo():
    ...
    try:
        ...
        print("repo not importable: skipping equivalence check"); return
    torch.manual_seed(0)
    cfg = LaplaceConfig(d=32, M=24, dv=8, L=8, ff=64, chunk=16)
    ref = build("cshort_damph", LayerCfg(32, 24, 4, 8, 64, freq="rope", theta_scale=0.02, dv=8, Ls=8, max_len=128)).double()
    mine = LaplaceAttention(cfg).double()
    sd = {k.replace("c.", "long.", 1) if k.startswith("c.") else k.replace("dh.", "short.", 1) if k.startswith("dh.") else k: v
          for k, v in ref.state_dict().items()}
    mine.load_state_dict(sd, strict=True)
    assert torch.allclose(mine.long.omega, ref.c.omega.double()) and torch.allclose(mine.short.omega, ref.dh.omega.double())
    B, T = 2, 45
    x = torch.randn(B, T, 32, dtype=torch.float64)
    ref.c.CTX = 16; ref.dh.CTX = 16
    with torch.no_grad():
        y_ref, _ = ref.prefill(x)
        for path in ("batched", "chunk"):
            mine.cfg.long_path = path
            y, _ = mine.prefill(x)
            print(f"  float64 vs repo, long_path={path:8s}: {(y - y_ref).abs().max().item():.2e}")
            assert (y - y_ref).abs().max() < 1e-11
        st = mine.init_state(B, x.device); ys = []
        for t in range(T):
            o, st = mine.step(x[:, t], st); ys.append(o)
        err = (torch.stack(ys, 1) - y_ref).abs().max().item()
        print(f"  float64 decode vs repo prefill:          {err:.2e}"); assert err < 1e-11
...
def _verify_iso(device):
    ...
    with torch.no_grad():
        y, _ = m.prefill(x)
        st = m.init_state(B, device); ys = []
        for t in range(T):
            o, st = m.step(x[:, t], st); ys.append(o)
        e1 = ((torch.stack(ys, 1) - y).abs().max() / y.abs().max()).item()
        y1, s1 = m.prefill(x[:, :77]); y2, _ = m.prefill(x[:, 77:], s1)
        e2 = ((torch.cat([y1, y2], 1) - y).abs().max() / y.abs().max()).item()
    print(f"  float32 on {device}: decode vs prefill rel {e1:.1e}, split 77|123 rel {e2:.1e}")
...
def _report_bf16(m, x, y_fp32):
    if not x.is_cuda:
        print("  bf16 report needs cuda: skipped"); return
    with torch.no_grad(), torch.autocast("cuda", dtype=torch.bfloat16):
        y, _ = m.prefill(x)
    rel = ((y.float() - y_fp32).abs().max() / y_fp32.abs().max()).item()
    print(f"  bf16 autocast vs float32 (big GEMMs in bf16, rest fp32): max rel dev {rel:.1e}")

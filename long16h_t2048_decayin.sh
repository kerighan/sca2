#!/usr/bin/env bash
# T=2048, 16h: dv=256 (base size) + lam_free + decay_input + conv_silu.
# decay_input makes the forgetting token-dependent: lam = exp(a + Wd(z)),
# like GDN's g = -exp(A_log) * softplus(a(x) + dt_bias). Combined with
# conv_silu (non-linearity before the state) and our base dv=256.
set -u
...
cell t2048_decayin --layer-scale --lam-free --damp-mem 4,20000 --gdn-gate --decay-input
echo "##### DONE"
